...
export default function DashboardPage() {
  ...
  const { name, age, salary, otherIncome, expenses, savingsGoal, riskProfile } = userData
  const totalIncome = salary + otherIncome
  const totalExp    = Object.values(expenses).reduce((a, b) => a + (Number(b) || 0), 0)
  const canSave     = totalIncome - totalExp
  const score       = calcScore(totalIncome, expenses, savingsGoal)
  const emergencyFund = totalExp * 6
  const monthsToEmergency = canSave > 0 ? Math.ceil(emergencyFund / canSave) : 0
          ...
          <div className="flex items-center gap-2">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs text-gray-400">
                Age {age} · {riskProfile.charAt(0).toUpperCase() + riskProfile.slice(1)} investor
              </p>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5">
            ...
            <p className={`text-xl sm:text-2xl font-black ${canSave >= 0 ? "text-green-700" : "text-red-500"}`}>
              {fmt(canSave)}
            </p>
